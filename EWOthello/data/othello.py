...
def get_data_path(data_root):
    resource_path = Path(__file__).parent / data_root
...
class Othello:
    def __init__(self, ood_perc=0.0, data_root=None, wthor=False, ood_num=1000, num_preload=10):
        ...
        self.sequences = []  # List of game sequences (converted to board number) across pgn files
        self.results = []  # list of game results across pgn files
        self.board_size = 8 * 8
        criteria = lambda fn: fn.endswith("pgn") if wthor else fn.startswith("liveothello")

        if data_root is None:  # Loads the synthetic game data
            if ood_num == 0:  # generic catch
                return
            else:
                datpath = get_data_path(wanna_use)
                if ood_num != -1:  # this setting used for generating synthetic dataset by playing random games
                    ...
                    for f in bar:
                        if not f.endswith(".pickle"):
                            continue
                        try:
                            with open(datpath.joinpath(f), "rb") as handle:
                                cnt += 1
                                if cnt > 250:  # Max files (?) or why cnt break
                                    break
        ...
        else:  # Load the profession game data
            datpath = get_data_path(data_root)
            for fn in os.listdir(datpath):
                if criteria(fn):
                    with open(datpath.joinpath(fn), "r") as f:
                        pgn_text = f.read()
                    games = pgn.loads(pgn_text)
                    num_ldd = len(games)

                    processed = []  #  list of sequential move tokens for each game
                    res = []  # List of end game board results ([# white, # black])

                    # Iterate game in pgn file
                    for game in games:
                        # For each game, tba holds dict value of board move played in sequence (append tba to processed)
                        tba = []
                        ...
                        # store the end game result (num white, num black) in res
                        if len(tba) != 0:
                            try:
...
class OthelloBoardState:
    ...
    def plot_hm(self, ax, heatmap, pdmove, logit=False):
        ...
        for loca, col in enumerate(ann_col):
            if col != "grey":
                patchList.append(PatchCollection([mpatches.Circle((loca % 8 + 0.5, loca // 8 + 0.5), 0.25, facecolor=col)], match_original=True))
        for i in patchList:
            ax.add_collection(i)
        return ax
